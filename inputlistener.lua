...
local mtCodes = {
  ...
  KOBO_STYLUS_MARKER = 332,
  KOBO_STYLUS_SENSOR = 24,
}
...
TouchEventType.PrintNames = {
  ...
  [TouchEventType.MARKER_DOWN] = "MARKER_DOWN",
  [TouchEventType.PRESSURE_DOWN] = "PRESSURE_DOWN",
}
...
ToolSubType.PrintNames = {
  ...
  [ToolSubType.MARKER] = "MARKER",
  [ToolSubType.PRESSURE] = "PRESSURE",
}
...
function InputListener:createTouchEvent(event, time)
  ...
  if event.subtool == ToolSubType.MARKER then
    logger.dbg("NW: Setting EraserDown")
    touchEventType = TouchEventType.MARKER_DOWN
  end
  if event.subtool == ToolSubType.PRESSURE then
    touchEventType = TouchEventType.PRESSURE_DOWN
  end
...
---@param hook_params any
-- local pressure = 3
function InputListener:pressureV()
...
logger.dbg(
    ...
    if event.code == mtCodes.KOBO_STYLUS_MARKER and event.value == 1 then
      Device.input:setCurrentMtSlotChecked("subtool", ToolSubType.MARKER)
      self.marker_id = self.current_tracking_id
    end 
    
    if event.code == mtCodes.KOBO_STYLUS_SENSOR then
      self.pressure = event.value
      Device.input:setCurrentMtSlotChecked("subtool", ToolSubType.PRESSURE)
      -- Device.input:setCurrentMtSlotChecked("subtool", ToolSubType.PEN)
    end
    if event.code == mtCodes.KOBO_STYLUS_MARKER and event.value == 0 then
      Device.input:setCurrentMtSlotChecked("subtool", ToolSubType.PEN)
    end
  end
end
